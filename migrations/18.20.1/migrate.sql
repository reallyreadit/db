...
-- Make the core schema explicit
CREATE OR REPLACE FUNCTION core.utc_now() RETURNS timestamp without time zone
    LANGUAGE sql STABLE
    AS $$
...
-- Make the core schema explicit
CREATE OR REPLACE FUNCTION core.generate_local_timestamp_to_utc_range_series(start timestamp with time zone, stop timestamp with time zone, step interval, time_zone_name text) RETURNS TABLE(local_timestamp timestamp with time zone, utc_range tsrange)
...
-- Make the core schema explicit
CREATE OR REPLACE FUNCTION stats.get_current_streak(user_account_id bigint) RETURNS stats.streak
    LANGUAGE sql STABLE
    AS $$
