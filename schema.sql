...
CREATE FUNCTION analytics.get_conversions(start_date timestamp without time zone, end_date timestamp without time zone) RETURNS TABLE(week timestamp without time zone, visit_count bigint, signup_count bigint, signup_conversion numeric, share_count bigint, share_conversion numeric, article_view_count bigint, article_view_conversion numeric, article_read_count bigint, article_read_conversion numeric, post_tweet_count bigint, post_tweet_conversion numeric)
    LANGUAGE sql STABLE
    AS $$
...
CREATE FUNCTION analytics.get_signups(start_date timestamp without time zone, end_date timestamp without time zone) RETURNS TABLE(id bigint, name text, email text, date_created timestamp without time zone, time_zone_name text, client_mode text, marketing_variant integer, referrer_url text, initial_path text, current_path text, action text, orientation_share_count bigint, article_view_count bigint, article_read_count bigint, post_tweet_count bigint)
    LANGUAGE sql STABLE
    AS $$
	SELECT
		user_account.id,
